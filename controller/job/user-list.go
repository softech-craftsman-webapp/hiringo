...
func GetMyJobs(ctx echo.Context) error {
	...
	db.Model(&model.Contract{}).Where("professional_id = ?", claims.User.ID).Pluck("job_id", &jobIDs).Order("created_at")
	db.Model(&model.Job{}).Where("id IN (?)", jobIDs).Find(&appliedJob).Order("created_at")

	// get created jobs
	db.Model(&model.Job{}).Where("user_id = ?", claims.User.ID).Find(&createdJob).Order("created_at")
